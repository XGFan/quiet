...
class PostService(private val quiet: Path, private val quietConfig: QuietConfig) {
    companion object {
    ...
    fun countPage(pageSize: Int): Int = posts.asSequence().filterNot { it.draft }
        .filter { it.categories.intersect(quietConfig.getHiddenDirs()).isEmpty() }.count()

    fun findPost(offset: Int, limit: Int): Page<Post> {
        val total = posts.asSequence().filterNot { it.draft }
            .filter { it.categories.intersect(quietConfig.getHiddenDirs()).isEmpty() }
        return total.drop(offset).take(limit) to total.count()
    }
    ...
    fun findPost(cats: List<String>): List<Post> =
        posts.asSequence().filterNot { it.draft }.filter { it.matchCat(cats) }.toList()
    ...
    private fun getDate(
        fileCreation: LocalDateTime,
        fileModify: LocalDateTime,
